...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/16 19:17:53 by faikhan           #+#    #+#             */
...
static void	open_output_file(t_fds *fds, char *filename, char *filename_cpy)
{
	...
	fds->fd_out = open(fds->output_file, O_WRONLY | O_CREAT | O_APPEND, 0664);
	if (fds->fd_out == -1)
...
void	parse_append(t_cmd **head_cmd, t_tok **head_tok)
{
	...
	init_fds(cmd);
	open_output_file(cmd->fds, (*head_tok)->nxt->data,
